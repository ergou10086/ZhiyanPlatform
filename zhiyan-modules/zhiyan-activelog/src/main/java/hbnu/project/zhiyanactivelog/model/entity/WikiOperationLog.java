 ...
 */
@Entity
@Table(name = "wiki_operation_log", indexes = {
...
public class WikiOperationLog {
     ...
     */
    @LongToString
    @Column(name = "wiki_page_id", columnDefinition = "BIGINT COMMENT 'Wiki页面ID（可为空，如批量操作）'")
    private Long wikiPageId;

    /**
     * Wiki页面标题（冗余字段，便于查询）
     */
    @Column(name = "wiki_page_title", length = 500, columnDefinition = "VARCHAR(500) COMMENT 'Wiki页面标题（冗余字段，便于查询）'")
     ...
     * 操作模块
     */
    @Column(name = "operation_module", nullable = false, length = 50, columnDefinition = "VARCHAR(50) NOT NULL DEFAULT 'Wiki管理' COMMENT '操作模块'")
    private String operationModule = "Wiki管理";
