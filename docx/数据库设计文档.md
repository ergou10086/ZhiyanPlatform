...
**核心功能**：用户身份管理、角色权限控制、验证码管理

**包含表**：`users`、`roles`、`user_roles`、`permissions`、`role_permissions`、`verification_codes`
...
CREATE TABLE `users` (
    ...
    `title` VARCHAR(100) DEFAULT NULL COMMENT '用户职称/职位',
    `institution` VARCHAR(200) DEFAULT NULL COMMENT '用户所属机构',
    `is_locked` TINYINT(1) DEFAULT '0' COMMENT '是否锁定（禁止登录）',
    ...
    `status` VARCHAR(20) NOT NULL COMMENT '用户状态（枚举：ACTIVE/LOCKED/DISABLED/DELETED）',
    PRIMARY KEY (`id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='系统角色定义表（用于权限分组）';
```
...
CREATE TABLE `permissions` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `name` (`name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='系统权限定义表（细粒度操作权限）';
```
...
CREATE TABLE `verification_codes` (
    ...
    PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='验证码管理表（用于注册、重置密码等场景）';
```
